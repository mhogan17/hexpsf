...
class HexPP:
    ...
    def get_HexPP(self, rho, theta):
        ...
        for i in range(len(theta)):
            ...
            alpha = 10
            if 0 < angle <= np.pi / 3:
                phase_plate[i] = self.actuator_heights[0] #* custom_sigmf(rho[i], alpha, 0)
            elif np.pi / 3 < angle <= 2 * np.pi / 3:
                phase_plate[i] = self.actuator_heights[1] #* custom_sigmf(rho[i], alpha, 0)
            elif 2 * np.pi / 3 < angle <= np.pi:
                phase_plate[i] = self.actuator_heights[2] #* custom_sigmf(rho[i], alpha, 0)
            elif np.pi / 3 < angle <= 4 * np.pi / 3:
                phase_plate[i] = self.actuator_heights[3] #* custom_sigmf(rho[i], alpha, 0)
            elif 4 * np.pi / 3 < angle <= 5 * np.pi / 3:
                phase_plate[i] = self.actuator_heights[4] #* custom_sigmf(rho[i], alpha, 0)
            else:
                phase_plate[i] = self.actuator_heights[5] #* custom_sigmf(rho[i], alpha, 0)
